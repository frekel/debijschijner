    ...
    <form method="post" action="{{ route('apply.submit') }}" class="cms-native-form" novalidate>
        ...
        <div class="cms-native-form-row cms-native-form-row-split">
            <div class="cms-native-form-field">
                ...
                <input id="apply-first-name" type="text" name="bsforms[fields][0][first]" value="{{ old('bsforms.fields.0.first') }}" required>
                @error('bsforms.fields.0.first')<p class="cms-native-form-error">{{ $message }}</p>@enderror
            </div>
            <div class="cms-native-form-field">
                <label for="apply-last-name">Achternaam</label>
                <input id="apply-last-name" type="text" name="bsforms[fields][0][last]" value="{{ old('bsforms.fields.0.last') }}" required>
        ...
        <div class="cms-native-form-honeypot" aria-hidden="true">
            ...
            <input id="apply-honeypot" type="text" name="bsforms[fields][2]" value="{{ old('bsforms.fields.2') }}" tabindex="-1" autocomplete="off">
        </div>

        <div class="cms-native-form-field">
            <label for="apply-email">Email</label>
            <input id="apply-email" type="email" name="bsforms[fields][1]" value="{{ old('bsforms.fields.1') }}" required>
            @error('bsforms.fields.1')<p class="cms-native-form-error">{{ $message }}</p>@enderror
        </div>

        <div class="cms-native-form-field">
            <label for="apply-phone">Telefoonnummer</label>
            <input id="apply-phone" type="text" name="bsforms[fields][9]" value="{{ old('bsforms.fields.9') }}" required>
            @error('bsforms.fields.9')<p class="cms-native-form-error">{{ $message }}</p>@enderror
        </div>

        <div class="cms-native-form-field">
            <label for="apply-trajectory">Traject</label>
            <select id="apply-trajectory" name="bsforms[fields][10]" required>
                @foreach(['Intakegesprek', 'Kort traject', 'Regulier traject', 'Anders, namelijk'] as $option)
                    <option value="{{ $option }}" @selected(old('bsforms.fields.10') === $option)>{{ $option }}</option>
                @endforeach
            </select>
            @error('bsforms.fields.10')<p class="cms-native-form-error">{{ $message }}</p>@enderror
        </div>

        <div class="cms-native-form-field">
            <label for="apply-message">Bericht</label>
            <textarea id="apply-message" name="bsforms[fields][3]" rows="2">{{ old('bsforms.fields.3') }}</textarea>
